...
class CommandStructureLexerTest {
    ...
    fun commandWithDelimiter() {
        ...
        assertTrue(nonWs.any { it.type == GdbTypes.ARG && it.text == "--" })
        assertTrue(nonWs.any { it.type == GdbTypes.ARG && it.text == "my_complex_command" })
        ...
        assertTrue(afterCommand.all { it.type == GdbTypes.ARG }, "All args after 'with' should be ARG tokens")
    }
    @Test
    fun commentsContinuationsCommandsSimpleTest() {
        val content = """
        ...
        val nonWs = tokens.filter { it.type != TokenType.WHITE_SPACE }
        assertTrue(nonWs.any { it.type == GdbTypes.COMMAND_BREAKPOINT && it.text == "bre" })
