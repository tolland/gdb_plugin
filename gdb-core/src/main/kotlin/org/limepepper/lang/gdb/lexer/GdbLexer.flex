...
%{
  ...
  private String getStateName(int state) {
    switch (state) {
      ...
      case STATE_D_STRING: return "STATE_D_STRING";
      case IS_PYTHON_INLINE: return "IS_PYTHON_INLINE";
      case IN_TEXT_BLOCK: return "IN_TEXT_BLOCK";
      case IN_ARGS: return "IN_ARGS";
      case STATE_COMMANDS_LIST: return "STATE_COMMANDS_LIST";
      case STATE_COMMENT_CONTINUATION: return "STATE_COMMENT_CONTINUATION";
      default: return "UNKNOWN_STATE_" + state;
    }
  }

  private void debugStackState(String operation) {
    if ("true".equals(System.getProperty("LEXER_DEBUG"))) {
      System.err.println(String.format("LEXER_DEBUG: %s - Current state: %s(%d), Stack size: %d, Stack: %s, Position: %d",
                                      operation, getStateName(yystate()), yystate(), stack.size(), stack.toString(), zzCurrentPos));
    }
  }

  private void resetStack() {
    if ("true".equals(System.getProperty("LEXER_DEBUG"))) {
      System.err.println("LEXER_DEBUG: RESETTING STACK - Previous stack: " + stack.toString());
    }
    stack.clear();
  }
  ...
  public void yypushState(int newState) {
    ...
    if (currentState == YYINITIAL && !stack.empty()) {
      System.err.println(String.format("Current state: %s(%d), New state: %s(%d), Stack size: %d, Stack contents: %s, Position: %d",
                                      getStateName(currentState), currentState, getStateName(newState), newState, stack.size(), stack.toString(), zzCurrentPos));
      resetStack();
    }
  ...
  public void yypopState() {
    ...
    if (stack.empty()) {
      System.err.println("WARNING: Attempting to pop from empty stack at position " + zzCurrentPos);
      return;
    }else {
      yybegin(stack.pop());
    }
